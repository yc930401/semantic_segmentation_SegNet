...
def data_generator(image_ids):
    ...
    for key in x_data.keys():
        print('Preparing image: {} -------- Image size: {} -------- Same size: {}'
        ...
        print(np.shape(x_image), np.shape(y_new_image))
        x.append(x_image)
        y.append(y_new_image)
    return x, y
...
if __name__ == '__main__':
    ...
    if os.path.exists("segnet.h5"):
        ...
        x, y = data_generator(train_ids[:3])
        for i in range(len(x)):
            model.fit(x[i], y[i], epochs=1, verbose=2)
